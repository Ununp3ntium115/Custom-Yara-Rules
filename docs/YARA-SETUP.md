...
This guide explains how Velociraptor Claw Edition updates and publishes automated YARA rules packages using crontab or GitHub Actions.

## YARA Rules Sources (7 Total)
...
| **GlasswormYARA** | Custom GlasswormYARA rules and IOC data | Variable |

## Custom-Yara-Rules Repository

Velociraptor Claw Edition publishes the generated YARA code and packages to a dedicated public repository every day.

| Item | Path |
|------|------|
| Source path in this repo | `yara-rules/` |
| Publish contract | `yara-rules/custom-repo/` |
| Public repository | `Ununp3ntium115/Custom-Yara-Rules` |
...
| Workflow | `.github/workflows/yara-rules-update.yml` |

## Automation Options

### Option 1: GitHub Actions (Recommended)

GitHub Actions automatically updates rules daily at 3 AM UTC, syncs the `Custom-Yara-Rules` repository code paths, and publishes both a dated release and the static `latest` package.

Required secret:

- `YARA_PUBLISH_TOKEN` - GitHub token with `contents:write` access to `Ununp3ntium115/Custom-Yara-Rules`
...
| `yara-rules/custom-repo/` | Custom-Yara-Rules publish contract |
...
3. **Velociraptor deployments** - Distribute to Velociraptor clients and servers managed by Claw Edition
